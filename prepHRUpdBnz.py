# copied from HR WLMT python modules *************** BNZ ***************
...
L = open('BnzHRChng.log', 'w')  # track items with price changes & qty showing old new pirce
## log file
#with open('DCQtyPrc.csv', 'r') as rf: # previosuly prepared with unitcost and unit price
header=["id","quantity","price","force_update"] # header BNZ
with open('mws/data/BNZ_HR_SKU_MASTER.txt', 'r') as rf: # our wlmt csv file built from API we rereate new to keep functionlaity
     reader = csv.reader(rf, delimiter='\t')
     with open('HRUpdBnz.csv', 'w') as wf:  # single file with both qty and prc for upload to bnz
          ...
          for line in reader:
              ...
              titleIn=line[1]  # only 2 columns in sku master txt for BNZ
              found=0
                ...
                for line in readerV:
                    ...
                    if skuHR == skuInx:  # compare skuin stripped without prefix
                        ...
                        if leadTI > maxLeadT:  # we force inactive , do not turn on
                            ...
                            qChngCtrOff +=1 
                            lines=[skuIn,qtyOutstr,str(priceV),"TRUE"]
                        ...
                        if qtyHRI >= minQty:
                            ...
                            qtyOut=0  # turn off
                            qChngCtrOff +=1 
                        #write one record with price and qty if we get here 
